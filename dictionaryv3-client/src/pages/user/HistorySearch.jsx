...
const HistorySearch = () => {
	...
	const getWordHistory = async () => {
		...
		try {
			...
			const result = await axios.post(`${import.meta.env.VITE_REACT_APP_API}/api/v1/auth/user-auth/search/${id}`, { searchedWord: history })
			...
			<Layout>
				<div className="word-container">
					...
					<Link to={"/user/history"}>
						<button className="bottom-right-button">Back</button>
					</Link>
				</div>
			</Layout>
		</>
